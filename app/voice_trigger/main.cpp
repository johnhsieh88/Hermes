...
// Own PipeWire raw-mic tap → sherpa-onnx KWD (C API) + PrerollRing writer.
...
static constexpr int      kRingCap  = kMicRate * 4;  // 4 s float32 ring
...
// ── Minimal SPSC float ring (PW RT thread → KWD thread) ──────────────────────
struct FloatRing {
    float              buf[kRingCap]{};
    ...
    void push(const float* src, int n) {
        ...
        for (int i = 0; i < n; ++i) { buf[w] = src[i]; w = (w+1) % kRingCap; }
        wp.store(w, std::memory_order_release);
    }

    int pop(float* dst, int n) {
        int av = (wp.load() - rp.load() + kRingCap) % kRingCap;
        if (av < n) n = av;
        int r = rp.load(std::memory_order_relaxed);
        for (int i = 0; i < n; ++i) { dst[i] = buf[r]; r = (r+1) % kRingCap; }
...
public:
    ...
    void Stop() {
        ...
        if (preroll_) { munmap(preroll_, sizeof(PrerollRing)); preroll_ = nullptr; }
    }
...
private:
    ...
    static PrerollRing* openPreroll() {
        ...
        close(fd);
        return (p == MAP_FAILED) ? nullptr : static_cast<PrerollRing*>(p);
    }
...
fallback:
    ...
    static void s_mic(void* u, float* samples, uint32_t nf, uint32_t) {
    ...
    std::thread        kwdThread_, pwThread_;
