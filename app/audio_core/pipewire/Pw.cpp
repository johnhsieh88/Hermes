...
PwStream::PwStream(PwClient& client, const char* name, Dir dir, StreamFn fn, void* user,
    ...
    pw_properties* props = pw_properties_new(
        ...
        nullptr);
    if (target) pw_properties_set(props, PW_KEY_NODE_TARGET, target);
    impl_->stream = pw_stream_new(client.core(), name, props);
...
int PwStream::connect(uint32_t sampleRate) {
    ...
    pw_direction pwdir = (impl_->dir == CAPTURE) ? PW_DIRECTION_INPUT : PW_DIRECTION_OUTPUT;
    return pw_stream_connect(impl_->stream, pwdir, PW_ID_ANY,
